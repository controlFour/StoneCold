...
void UIStateMachine::handlePIDMenuMode(int delta) {
    ...
    // Wrap around and skip SAVE if no changes
    do {
        if (newSelection < 0) {
        ...
        // Skip SAVE if no changes pending
        if (newSelection == PID_MENU_SAVE && !_pidSettingsChanged) {
            newSelection += delta;
            if (newSelection < 0) newSelection = PID_MENU_ITEM_COUNT - 1;
            else if (newSelection >= PID_MENU_ITEM_COUNT) newSelection = 0;
        }
    } while (newSelection == PID_MENU_SAVE && !_pidSettingsChanged);
